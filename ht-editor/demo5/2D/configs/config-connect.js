...
ht.DataModel.prototype.deserialize = function(u, o, W){
	...
	try{
		if(!window.htUserInfo || !window.htUserInfo.logo){
			if(!window.htUserInfo){window.htUserInfo = {}};
			window.htUserInfo.logo = true;
			...
			if(u.contentRect && u.contentRect.width && u.contentRect.height){
				var par = Math.min(u.contentRect.width/200,u.contentRect.height/53);
				if(par < 1){
					logoNode.setSize(200*par/2, 53*par/2);
					logoY = logoY - 53*par/2;
					logoX = logoX + 200*par/2;
				}else{
