(function() {
    ...
    function handleEditorCreated(editor) {
        ...
        editor.assets.accordion.menu.setItemVisible('convertToDisplay', false);
		editor.assets.accordion.menu._items.push(createPublicAssetItem());
        // Add print item
	...
	//发布到公共资源
	function createPublicAssetItem(){
		var item = {
			id: "publicAsset",
			label: '发布到公共资源',
			action: function() {
				var node = editor.assets.accordion.sm().ld();
				sendAssetToPublic();
			},
			visible: function() {
				var q = editor.assets.getFileListView().sm().ld();
